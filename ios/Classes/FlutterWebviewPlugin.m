...
- (void)navigate:(FlutterMethodCall*)call {
    if (self.webview != nil) {
            ...
            if ( [withLocalUrl boolValue]) {
                ...
                NSString *urlConvert = (NSString*)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)url, (CFStringRef)@"!$&'()*+,-./:;=?@_~%#[]", NULL, kCFStringEncodingUTF8));
                NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlConvert]];
                NSLog(@"--------URL:%@----request：%@", [NSURL URLWithString:url], request);
                NSDictionary *headFields = request.allHTTPHeaderFields;
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction
    ...
    NSLog(@"-----重定向-------1----url:%@-------%@", navigationAction.request.URL, data);
